-- Fix the wallet update trigger to handle payments that are already completed when inserted
...
-- Also create an INSERT trigger since we need to handle payments that are completed when inserted
