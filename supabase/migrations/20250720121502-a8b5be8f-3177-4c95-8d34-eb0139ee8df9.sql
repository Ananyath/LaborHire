...
INSERT INTO public.platform_settings (setting_key, setting_value, description, updated_by) VALUES
('platform_name', '"LaborLink"', 'Platform name displayed across the application', (SELECT id FROM public.profiles WHERE user_id = auth.uid() LIMIT 1)),
('commission_rate', '0.05', 'Platform commission rate (5%)', (SELECT id FROM public.profiles WHERE user_id = auth.uid() LIMIT 1)),
('min_payment_amount', '5.00', 'Minimum payment amount allowed', (SELECT id FROM public.profiles WHERE user_id = auth.uid() LIMIT 1)),
('max_payment_amount', '10000.00', 'Maximum payment amount allowed', (SELECT id FROM public.profiles WHERE user_id = auth.uid() LIMIT 1)),
('auto_approve_workers', 'false', 'Automatically approve worker registrations', (SELECT id FROM public.profiles WHERE user_id = auth.uid() LIMIT 1)),
('require_identity_verification', 'true', 'Require identity verification for workers', (SELECT id FROM public.profiles WHERE user_id = auth.uid() LIMIT 1));
